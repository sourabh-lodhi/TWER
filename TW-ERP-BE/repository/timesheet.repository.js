  ...
  getTimesheetDetails = async (timesheetDetail, salary_password) => {
    ...
    if (timesheetDetail) {
      ...
      for (let i = 0; i < sheetData.length; i = i + 10) {
        ...
        const element = Object.values(sheetData[i]);
        const empCode = element[2];
        let presents = Number(element[14]);
        let absents = Number(element[23]);
        ...
        if (user) {
          ...
          let total_week_offs = Number(element[16]);
